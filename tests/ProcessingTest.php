...
final class ProcessingTest extends TestCase
{
    ...
    {
        $flag = new TaskFlag();
        $task = $this->taskQuery->getOne(
            new EntityUuid($this->task->getUuid())
        );

        self::assertEquals($flag->setReady()->toValue(), $task->flag);

        self::assertFalse($this->processing->hasTaskProcess());
        $this->processing->loadTaskProcess(
            $this->options
        );
        ...
        self::assertEquals($flag->setRunning()->toValue(), $stage->flag);
